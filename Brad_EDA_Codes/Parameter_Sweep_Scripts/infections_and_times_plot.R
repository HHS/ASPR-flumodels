...
p1 <- p1 + ggtitle("Peak Infection Value (fractional) as a Function of ti, tL, and R0")

p1 <-p1 + scale_color_gradientn(colours = c("green","orange","red"), 
                                values = rescale(c(0,90,365)),
                                guide = "colorbar", limits=c(0,365))


p1 <- p1 + scale_color_gradient2(midpoint=0.5, low="blue", mid="yellow",
                      high="red", space ="Lab",name = "I_peak" )
...
p1 <-p1 + scale_color_gradientn(colours = c("red","orange","yellow","green","blue"), 
                             values = rescale(c(0,30,60,120,180)),
